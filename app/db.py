...
def connect_to_db_as_user():
    try:
        conn = psycopg2.connect(
            user = current_app.config['DATABASE_USER_U'],
            password = current_app.config['DATABASE_PASSWORD_U'],
            dbname = current_app.config['DATABASE_NAME'],
            host= current_app.config['DATABASE_HOST'],
            port= current_app.config['DATABASE_PORT']
        )
...
def connect_to_db_as_admin():
    try:
        conn = psycopg2.connect(
            user = current_app.config['DATABASE_USER_A'],
            password = current_app.config['DATABASE_PASSWORD_A'],
            dbname = current_app.config['DATABASE_NAME'],
            host= current_app.config['DATABASE_HOST'],
            port= current_app.config['DATABASE_PORT']
        )
